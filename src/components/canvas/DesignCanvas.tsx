...
export const DesignCanvas = forwardRef<any, DesignCanvasProps>(({
  ...
  const loadShirt = useCallback(async () => {
    ...
    fabric.loadSVGFromURL(url, (objects: any[], options: any) => {
      ...
      const scaleX = (CANVAS_W * 0.82) / (group.width ?? 280);
      const scaleY = (CANVAS_H * 0.88) / (group.height ?? 320);
      ...
      // Remove old shirt & printable
      const toRemove = canvas.getObjects().filter((o: any) =>
        ["tshirt", "printable"].includes(o.name)
      );
      ...
      const pw = CANVAS_W * area.w;
      const ph = CANVAS_H * area.h;
  ...
  useEffect(() => {
    if (
      isReady &&
      (prevStyle.current !== shirtStyle ||
        prevColor.current !== shirtColor ||
        prevSide.current !== viewSide)
    ) {
      prevStyle.current = shirtStyle;
      prevColor.current = shirtColor;
      prevSide.current = viewSide;
  ...
  const addText = useCallback(async (options: {
    ...
    const cx = printable ? printable.left : CANVAS_W / 2;
    const cy = printable ? printable.top : CANVAS_H / 2;
    const maxW = printable ? printable.width * 0.9 : 200;

    const text = new fabric.IText(options.content, {
      left: cx,
      top: cy,
      originX: "center",
      originY: "center",
    ...
    if ((text.width ?? 0) > maxW) {
      text.set("scaleX", maxW / (text.width ?? 1));
    }
  ...
  return (
    <div className="relative flex items-center justify-center w-full h-full">
      ...
      <div
        className="absolute bottom-6 left-1/2 -translate-x-1/2 text-xs px-3 py-1 rounded-full"
        style={{
          background: "rgba(147,112,219,0.15)",
          border: "0.5px solid rgba(147,112,219,0.4)",
