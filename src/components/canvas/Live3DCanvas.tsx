...
export const Live3DCanvas = forwardRef<any, Live3DCanvasProps>(({
  ...
  const attachDecal = useCallback(async (THREE: any, group: any, tex: any) => {
    ...
    const printW = 1.2;
    const printH = 0.9;
    const chestY = center.y - 0.35;  // lower to mid-chest
    ...
    try {
      ...
      if (data.type === "text") {
        const text = new fabric.Textbox(data.content || "Text", {
          ...
          fontSize: data.fontSize ? data.fontSize / 2 : 40,
  ...
  // Forward mouse events from 3D canvas to Fabric canvas for editing
  const handleCanvasClick = useCallback((e: React.MouseEvent) => {
    const fc = fabricCanvasRef.current;
    if (!fc) return;
    const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
    const x = ((e.clientX - rect.left) / rect.width) * TEX_SIZE;
    const y = ((e.clientY - rect.top) / rect.height) * TEX_SIZE;
    // Simulate click on Fabric canvas at the mapped position
    fc.discardActiveObject();
    const found = fc.getObjects().find((obj: any) => obj.containsPoint({ x, y }));
    if (found) {
      fc.setActiveObject(found);
    ...
    fc.renderAll();
    if (syncToTextureRef.current) syncToTextureRef.current();
  }, [fabricCanvasRef, onSelectObject, pauseAutoRotate]);
